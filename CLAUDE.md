...
- [x] Widget Studio "Video Scenario" (`SCENARIO_BLOG`): idee libre -> scene + script rediges par Claude (`server/utils/scenarioScriptGenerator.js`, editable avant generation), persona choisie comme simple proprietaire du contenu par defaut, case "Verrouiller l'identite" optionnelle (desactivee par defaut) qui bascule Omni Flash entre flux 1 tour (libre) et 2 tours (identite verrouillee, defauts connus). Reutilise l historique de versions existant pour comparer les modeles plutot que de construire une UI de comparaison dediee.
...
- [ ] Widget "Video Scenario" jamais eprouve sur une vraie generation payante depuis l interface (teste jusqu ici en compilation + HMR uniquement) -- premier test reel a faire avant un usage recurrent
