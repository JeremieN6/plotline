...
### Prochaines etapes :
- [ ] Terminer et fusionner le systeme de widgets Studio (branche `feature/studio-widgets`): prompts pre-remplis par template pour 4 cas d usage (portrait, vlog, food ad, UGC produit) -- code ecrit et teste unitairement, verification manuelle du flux complet dans le navigateur encore a faire avant fusion
...
- [ ] Eprouver `POST /api/external/video-jobs` (branche `feature/external-video-jobs`) avec un vrai appel Omni Flash: poser `EXTERNAL_VIDEO_JOBS_API_KEY` sur le VPS, verifier que le modele `gemini-omni-1.1-flash` est active sur le projet Google Cloud, et installer une police (`fonts-dejavu` ou `DRAWTEXT_FONT_PATH`) pour que l overlay ffmpeg ne soit pas silencieusement ignore
...
- `POST /api/external/video-jobs` (branche `feature/external-video-jobs`) est un point d entree service-a-service sans session, protege par `EXTERNAL_VIDEO_JOBS_API_KEY` (cle partagee inventee, a generer et poser identiquement sur le VPS Plotline et sur l appelant). Si la variable est absente, l endpoint refuse tout en 503 plutot que de s ouvrir sans protection -- ne jamais retirer cette garde. L overlay ffmpeg drawtext qu il declenche ne bloque pas la generation si aucune police n est trouvee sur le serveur: la video part sans le texte plutot que de perdre une generation Omni Flash couteuse.
...
| 2026-09-07 | L overlay ffmpeg du texte d accroche (hookVideo) ne bloque jamais la publication de la video si la police est introuvable sur le serveur: la video part sans le texte, l erreur est journalisee | Une generation Omni Flash est couteuse; perdre la video entiere pour un probleme de police serait une regression plus grave que l absence ponctuelle de surimpression |
