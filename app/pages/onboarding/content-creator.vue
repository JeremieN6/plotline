  ...
  <div class="min-h-screen bg-[#0C0A08] text-[#FFF5EB] px-4 py-10 sm:px-8">
    <div class="mx-auto max-w-3xl rounded-[24px] border border-[#3A2A1E] bg-[#120C07] p-6 sm:p-8">
      ...
      <div class="flex items-start justify-between gap-4">
        <div>
          <p class="text-xs uppercase tracking-[0.2em] text-[#F2B582]">Nouveau profil de production</p>
          <h1 class="mt-2 text-3xl font-black tracking-tight">{{ step === 1 ? 'Ton profil' : step === 2 ? 'Ambassadeur-rice (optionnel)' : 'Confirmation' }}</h1>
        </div>
        <NuxtLink
          to="/studio"
          class="flex h-8 w-8 shrink-0 items-center justify-center rounded-full border border-[#5B4332] text-[#E9C9AE] transition-colors hover:border-[#E8873A] hover:text-[#E8873A]"
          title="Fermer"
        >
          ✕
        </NuxtLink>
      ...
      <div v-if="step === 1" class="mt-8 space-y-5">
        <!-- Sélecteur de type -->
        <div>
          <p class="mb-2 text-sm font-semibold">Quel type de profil crées-tu ?</p>
          <div class="grid grid-cols-3 gap-2">
            <button
              v-for="t in profileTypes"
              :key="t.value"
              type="button"
              class="flex flex-col items-center gap-1.5 rounded-[12px] border px-3 py-3 text-center transition-all"
              :class="form.profileType === t.value
                ? 'border-[#E8873A] bg-[#E8873A]/10 text-[#FFF5EB]'
                : 'border-[#5B4332] bg-[#1A120D] text-[#E9C9AE] hover:border-[#E8873A]/50'"
              @click="form.profileType = t.value"
            >
              <span class="text-xl">{{ t.icon }}</span>
              <span class="text-xs font-bold">{{ t.label }}</span>
              <span class="text-[10px] leading-tight text-[#A88060]">{{ t.hint }}</span>
            </button>
          </div>
        </div>
        ...
        <div class="space-y-4">
          <div>
            <label class="block text-sm font-semibold">{{ namePlaceholderLabel }}</label>
            <input v-model="form.name" class="mt-1.5 w-full rounded-[12px] border border-[#5B4332] bg-[#1A120D] px-4 py-3 text-white outline-none focus:border-[#E8873A]" :placeholder="namePlaceholder" />
          </div>

          <div>
            <label class="block text-sm font-semibold">Ta niche</label>
            <input v-model="form.niche" class="mt-1.5 w-full rounded-[12px] border border-[#5B4332] bg-[#1A120D] px-4 py-3 text-white outline-none focus:border-[#E8873A]" placeholder="Ex : business, éducation, lifestyle, tech..." />
          </div>

          <div>
            <label class="block text-sm font-semibold">Ton style de contenu</label>
            <input v-model="form.style" class="mt-1.5 w-full rounded-[12px] border border-[#5B4332] bg-[#1A120D] px-4 py-3 text-white outline-none focus:border-[#E8873A]" placeholder="Ex : minimaliste, coloré, premium..." />
          </div>
        </div>
      </div>
...
const namePlaceholderLabel = computed(() => {
  if (form.profileType === 'brand') return 'Nom de la marque'
  if (form.profileType === 'activity') return 'Nom de ton activité'
