...
### 2026-09-16 Ajouter un `import` ESM a un fichier qui garde `module.exports` casse le serveur de dev (mais pas le build de prod)
**Probleme** : apres avoir ajoute `import { ... } from '../../utils/contentPlanner.js'` en haut de `server/api/profiles/[id].get.js` et `[id].patch.js` (qui se terminaient chacun par `module.exports = defineEventHandler(...)`), `npm run build` passait sans aucune erreur (exit 0, bundle Rollup genere), mais le serveur de dev (`nuxt dev`) renvoyait une 500 sur absolument toutes les routes: `Cannot access 'renderer' before initialization`, precedee dans les logs par `[uncaughtException] module is not defined in ES module scope`.
**Cause racine** : Rollup (utilise par `npm run build`) transpile et tolere le melange `import`/`module.exports` dans un fichier, donc le build de prod ne detecte rien. Le SSR dev de Nitro/Vite, lui, charge ces fichiers comme de vrais modules ESM Node: la simple presence d un `import` en haut de fichier fait que Node traite tout le fichier en mode ESM strict, ou `module` (global CommonJS) n existe pas -- `module.exports` plus bas leve une exception non interceptee qui casse l enregistrement de TOUTES les routes du serveur, pas seulement celle du fichier fautif.
**Solution** : remplacer `module.exports = defineEventHandler(...)` par `export default defineEventHandler(...)` dans les deux fichiers, pour rester coherent avec l `import` ESM ajoute.
**Regle** : avant d ajouter un `import ... from` statique en haut d un fichier serveur existant, verifier d abord comment ce fichier exporte son handler (`grep module.exports`). S il utilise encore `module.exports`, soit le convertir en `export default` au meme moment, soit garder l import en dynamique (`await import(...)`) comme le fait deja tout le reste du fichier -- ne jamais supposer qu un `npm run build` reussi valide la compatibilite avec le serveur de dev: les deux utilisent des bundlers differents avec une tolerance differente au melange CJS/ESM.
...
### 2026-09-06 Changer la signature d une fonction partagee sans verifier les branches mises de cote
**Probleme** : `getBodyBlock(silhouette, topGarment)` est devenue `getBodyBlock(silhouette, gender, topGarment)` sur la branche du genre du persona. `server/utils/personaDescription.js`, ecrit avant ce changement sur une autre branche (`feature/studio-widgets`, mise de cote par `git stash` pendant ce travail), appelait encore l ancienne forme -- passer le genre en 2e argument y aurait silencieusement pollue le texte de generation avec la valeur du genre a la place du nom du vetement.
**Cause racine** : un grep ou une recherche d appelants sur le seul repertoire de travail courant ne voit pas le code qui existe uniquement dans un stash ou sur une autre branche non fusionnee. Le probleme etait deja identifie et note au moment du changement de signature, mais seulement corrige au moment de reprendre l autre branche.
**Solution** : avant de fusionner main dans la branche mise de cote (et avant de reprendre le stash), verifier explicitement chaque appelant connu de la fonction dont la signature a change, meme ceux qui n existent que dans du travail non commite ailleurs.
**Regle** : changer la signature d une fonction partagee (`server/utils/*`) exige de verifier tous ses appelants sur TOUTES les branches et stashes en cours, pas seulement le repertoire de travail courant. Noter explicitement le point de suivi (ici: dans le plan de la fonctionnalite et les notes de session) et le traiter des la reprise de l autre branche, avant tout autre changement.

### 2026-09-07 Omni Flash ne s appelle pas comme les autres modeles Gemini
**Probleme** : appeler `gemini-omni-1.1-flash` avec les methodes habituelles du SDK (`ai.models.generateContent`, ou `ai.models.generateVideos` comme pour Veo) echoue -- le modele n existe pas sur ces endpoints, meme avec une cle API valide et le modele bien active sur le projet Google Cloud.
**Cause racine** : Omni Flash ne vit pas sur l API `models` classique. Il n est accessible que via l Interactions API, une surface distincte du SDK `@google/genai` (`ai.interactions.create(...)`, `ai.interactions.get(...)`), avec son propre format de requete (`input` accepte du texte et/ou des `ImageContent`/`VideoContent`, `response_format: { type: 'video', aspect_ratio }`, `previous_interaction_id` pour chainer plusieurs tours). Rien dans le nom du modele ne l indique -- l erreur ressemble a un probleme d activation ou de nom de modele mal orthographie, pas a une histoire de mauvais endpoint.
**Solution** : `server/utils/videoGeneration.js` (`requestOmniFlashSingleTurn`/`requestOmniFlashTwoTurn`) appelle exclusivement `ai.interactions.create`/`ai.interactions.get`, jamais `ai.models.*`, pour ce modele.
**Regle** : Avant de retoucher l integration Omni Flash, verifier que l appel passe bien par `ai.interactions.*`. Si quelqu un (humain ou agent) reintroduit un appel via `ai.models.generateContent`/`generateVideos` pour ce modele en pensant unifier le code avec Veo/Gemini image, ca echouera silencieusement en 404/"model not found" -- ce n est pas un signal d indisponibilite du modele, juste le mauvais point d entree du SDK.

### 2026-09-07 Endpoint service-a-service qui pointe vers une route protegee par session
**Probleme** : `POST /api/external/video-jobs` (nouveau point d entree sans session, authentifie par cle partagee, pour que home.sassify.fr pousse des jobs video) documentait `GET /api/content/:id/status` comme moyen de suivre l avancement. Au premier test reel, cet appel renvoyait `401 Authentification requise`.
**Cause racine** : `/api/content/:id/status` appelle `requireAuthUser` et scope sa requete par `userId` -- il est concu pour l interface du produit, ou un utilisateur connecte consulte ses propres contenus. Un appelant service-a-service n a pas de session ni de `userId`: cette route lui est structurellement inaccessible, quel que soit ce qu on lui envoie.
**Solution** : Ajout de `GET /api/external/video-jobs/:id/status`, jumeau de la route existante mais authentifie par la meme cle partagee que la creation du job, sans scoping utilisateur.
**Regle** : Quand un nouveau point d entree externe (sans session) doit s appuyer sur un flux existant pense pour l UI (polling de statut, webhook, etc.), ne jamais supposer que la route existante est reutilisable telle quelle -- verifier explicitement son mecanisme d authentification avant d ecrire la documentation ou le code appelant, et prevoir un jumeau si elle est session-scoped.

### 2026-09-08/09 Image de depart transmise sans jamais brancher le lip-sync
**Probleme** : premiere version d Omni Flash avec identite verrouillee -- image de depart + texte demandant de parler dans le meme appel -- produisait bien le bon visage mais une voix off, la bouche du personnage ne bougeant jamais.
**Cause racine** : Omni Flash ne synchronise les levres que via son mecanisme documente "multi-turn voice extension" (`previous_interaction_id`, qui etend une interaction VIDEO precedente), jamais quand une image est simplement jointe au meme tour qu une demande de parole. Rien dans la doc ne le dit explicitement en ces termes -- il a fallu confirmer par un test reel apres avoir lu la doc officielle en detail.
**Solution** : flux a deux tours (`requestOmniFlashTwoTurn`) -- tour 1 genere une scene silencieuse a partir de l image de depart, tour 2 chaine dessus via `previous_interaction_id` en demandant la parole. Fonctionne pour l identite et le lip-sync, mais introduit ses propres defauts (derive d identite entre tours, artefacts de raccord en debut/fin de clip) non resolus par ajustement de prompt.
**Regle** : Avant de conclure qu un modele "ne sait pas faire" quelque chose, verifier s il existe un mecanisme multi-tours/chaine distinct d un appel unique -- la doc peut decrire une capacite sans jamais preciser qu elle ne s applique qu a une forme d appel precise. Et un mecanisme qui resout un defaut peut en introduire d autres: valider chaque nouvelle approche par un test reel complet, pas seulement sur le defaut qu elle visait a corriger.

### 2026-09-09 Message dynamique corrige sur une branche, oubliee sur l autre
**Probleme** : l encart explicatif sous la case "Verrouiller l identite" du widget Video Scenario annoncait des consequences propres a Omni Flash (cout double, risque de derive) quel que soit le modele video reellement choisi dans le select juste au-dessus. Corrige une premiere fois pour la branche "case cochee" (message different selon `widgetVideoModel === 'omniflash'`) -- puis le meme defaut a ete signale une seconde fois par l utilisateur, cette fois sur la branche "case decochee", qui mentionnait encore "sur Omni Flash" en dur quel que soit le modele choisi.
**Cause racine** : un texte conditionnel avec plusieurs etats (ici : coche/decoche x modele choisi) a ete corrige etat par etat au fil des retours, au lieu d etre repense comme une matrice complete des qu un premier cas a revele le probleme. Corriger UNE branche d un `v-if`/`v-else` sans revenir sur les branches voisines laisse le meme bug ailleurs dans le meme bloc.

### 2026-09-10 Des fichiers tout juste ecrits semblaient avoir disparu -- en fait stashes par une session en parallele
**Probleme** : en plein milieu de l implementation de l assistant Claude autour du prompt, `npm test` a echoue avec `ERR_MODULE_NOT_FOUND` sur un fichier venant d etre cree avec succes (`server/utils/promptAssistGenerator.js`). Verification : le fichier, et 7 autres nouvellement crees, n existaient plus du tout sur le disque -- aucune trace, meme `git fsck` sur les blobs unreachable ne les retrouvait au premier abord.
**Cause racine** : l utilisateur (ou une session Claude Code en parallele) travaille dans CE MEME dossier de travail, pas un clone separe. Pendant l implementation, cet autre acteur a change de branche plusieurs fois dans ce dossier partage (`main` -> une branche vide deja nommee pour cette meme feature -> une branche de bugfix sans rapport), ce qui a emporte les fichiers non commites de la session en cours. Avant de changer de branche, cet acteur a correctement mis ce travail de cote via `git stash --include-untracked`, avec un message explicite nommant la branche cible -- un geste prudent, pas une perte de donnees, mais invisible depuis les seuls outils d edition de fichiers (Write/Edit ne savent pas qu un `git checkout` externe a eu lieu).
**Solution** : ne jamais supposer une perte de donnees sans verifier `git stash list`, `git reflog` et `git fsck --unreachable --dangling` d abord. `git stash show -p --stat` seul n affiche PAS les fichiers untracked meme stashes avec `-u` -- il faut `--include-untracked` sur la commande `show`, ou lire l arbre du 3e parent (`git ls-tree -r stash@{0}^3`), pour confirmer qu ils y sont vraiment avant de conclure a une perte. Une fois confirme, demander a l utilisateur avant de rebasculer de branche/pop le stash (le dossier etant partage, une bascule de branche affecte aussi ce que l autre acteur voit et tout serveur de dev qu il aurait en cours).
**Regle** : dans ce projet, le dossier de travail est potentiellement partage avec l utilisateur ou une autre session en parallele -- un `git status`/`git log`/`git reflog` peut changer sous les pieds d une session sans qu elle ait rien fait elle-meme. Si des fichiers venant d etre ecrits semblent absents, chercher un stash ou un changement de branche avant de re-ecrire quoi que ce soit ou d alarmer l utilisateur d une perte reelle.
**Regle** : Quand un texte affiche conditionnellement depend de PLUSIEURS variables d etat (ici : case a cocher + modele selectionne), lister explicitement toutes les combinaisons avant d ecrire le template, plutot que de corriger la premiere combinaison signalee et supposer que les autres sont deja correctes. Un `v-else` generique qui "marchait avant" peut rester faux pour des sous-cas qu il ne couvrait jamais correctement.

### 2026-09-13/14 Bruit de fond 24h/24 qui epuise un quota gratuit independamment du trafic reel
**Probleme** : le quota mensuel gratuit de Neon (CU-hours) et d Upstash Redis (commandes) s epuisaient bien avant la fin du mois, alors que le volume reel de generations et de publications restait tres faible.
**Cause racine** : deux boucles tournaient en continu, 24h/24, a des intervalles par defaut non repenses pour un usage a faible trafic. Le tick du scheduler de publication (`server/plugins/scheduler.ts`) interrogeait Neon toutes les 60s des que `SCHEDULER_ENABLED=true` -- plus frequent que le delai de mise en veille de la base, qui ne s endormait donc jamais et facturait du compute en continu. Le worker BullMQ (`server/utils/generationWorker.js`) gardait les intervalles par defaut de la lib (`drainDelay`/`stalledInterval` a 30s), interrogeant Upstash en boucle meme sans aucun job en file.
**Solution** : les deux intervalles sont passes a des valeurs bien plus larges (10 minutes pour le scheduler, 5 minutes pour le worker) -- un job reel reste pris en charge immediatement (la lecture bloquante de la file se reveille des qu un job y est ajoute), seules les relances "a vide" sont espacees.
**Regle** : reprendre l intervalle par defaut d une librairie (cron, queue, polling) sans se demander a quelle frequence ca interroge un service externe facture/quota a l usage est un piege classique sur un compte a faible trafic mais free-tier serre : le cout ne vient pas du volume de travail reel mais du bruit de fond de la boucle elle-meme. Verifier explicitement la frequence de tout polling/tick avant de l activer en continu (`SCHEDULER_ENABLED`, worker toujours actif, etc.).

### 2026-09-16 `localhost` renvoie 426 Upgrade Required sur cette machine de dev, `127.0.0.1` fonctionne
**Probleme** : en testant Plotline en navigateur (Browser pane) contre un `npm run dev` local frais, `http://localhost:<port>/` renvoyait systematiquement `426 Upgrade Required` (corps de 16 octets, aucune trace cote logs Nitro) -- confirme hors du panneau navigateur via `curl` direct, donc pas un artefact de proxy de l outil.
**Cause racine** : non elucidee avec certitude, mais `http://127.0.0.1:<port>/` sur le MEME process repond normalement (200, contenu Nuxt correct) -- la difference se joue donc entre `localhost` et `127.0.0.1`, probablement une resolution `localhost` vers IPv6 (`::1`) qui ne tape pas le meme bind que le serveur (ecoute en IPv4 uniquement).
**Solution** : utiliser `http://127.0.0.1:<port>` plutot que `http://localhost:<port>` pour toute verification navigateur en local sur cette machine.
**Regle** : si un serveur de dev local repond correctement au premier `curl`/logs (pas d erreur serveur) mais que le navigateur affiche une erreur bizarre comme "426 Upgrade Required" ou un refus de connexion incoherent avec les logs, tester `127.0.0.1` a la place de `localhost` avant de chercher une cause applicative -- ca peut n etre qu une resolution DNS locale qui ne tape pas le bind du serveur.

### 2026-09-23 Une regle `.gitignore` sur un dossier (`/tmp`) rend toute exception plus bas inoperante
**Probleme** : `!/tmp/exclusive-content/` et `!/tmp/exclusive-content/**` ajoutes juste apres `/tmp` dans `.gitignore` n avaient aucun effet -- `git check-ignore -v` continuait de rapporter le fichier comme ignore par la regle `/tmp` elle-meme, jamais par les exceptions.
**Cause racine** : quand un PATTERN DE REPERTOIRE (`/tmp`, sans wildcard) matche, Git exclut le repertoire entier et ne descend plus jamais dedans pour evaluer d autres regles -- documente dans `git help gitignore` ("It is not possible to re-include a file if a parent directory of that file is excluded"). Les exceptions `!...` plus bas etaient donc syntaxiquement correctes mais jamais evaluees.
**Solution** : remplacer `/tmp` par `/tmp/*` (matche le CONTENU du dossier, pas le dossier lui-meme) -- Git continue de descendre dans `/tmp`, et les exceptions `!/tmp/exclusive-content/` + `!/tmp/exclusive-content/**` s appliquent alors normalement.
**Regle** : pour exempter un sous-chemin d un dossier par ailleurs ignore, ne jamais ignorer le dossier avec un pattern nu (`/dossier`) si une exception y sera ajoutee plus tard -- utiliser `/dossier/*` des le depart. Verifier toute exception gitignore avec `git check-ignore -v <chemin>` (le pattern qui matche en dernier est affiche) avant de supposer qu elle marche.

### 2026-09-24 Un parametre expose et protege cote API n est pas un parametre utilisable si aucun ecran ne l envoie
**Probleme** : la premiere version de la bibliotheque de patterns de prompts (`server/data/promptPatterns.js`) exposait `patterns` dans `GET /api/widgets` et acceptait `patternId` sur `POST /api/widgets/resolve` et `POST /api/widgets/fields-assist`, avec verification serveur correcte (compte, widget). Mais `app/composables/useWidgets.js` ignorait `response.patterns`, et ni `resolveWidget()` ni `assistWidgetFields()` n envoyaient `patternId`. Resultat : les patterns "palier A" (les seuls a avoir un vrai interet manuel) etaient correctement proteges mais totalement inaccessibles depuis le Studio -- personne n aurait jamais pu les choisir, sans qu aucune erreur ne se produise nulle part (le flux par defaut continuait de marcher normalement).
**Cause racine** : le travail serveur (donnees, selection, garde-fous) a ete construit et teste de bout en bout (75 -> 81 tests), mais aucun test ni verification manuelle ne portait sur le CHEMIN COMPLET utilisateur -- "l API le permet" a ete pris pour "la fonctionnalite marche" sans verifier que l UI appelante avait ete mise a jour en consequence.
**Solution** : `useWidgets()` expose desormais `patterns` (charges depuis la reponse de `/api/widgets`), un select "Recette (optionnel)" filtre par widget est ajoute dans `studio/index.vue`, et `patternId` est transmis aux deux endpoints. Changer de widget ou de pattern reinitialise les champs deja remplis (une recette differente peut avoir des consignes differentes pour les memes cles).
**Regle** : quand une fonctionnalite ajoute un parametre optionnel a des endpoints existants (ici `patternId`), verifier explicitement TOUS les appelants front de ces endpoints (`grep` des composables/pages qui les invoquent) et confirmer qu au moins un chemin UI reel envoie ce parametre -- sinon la fonctionnalite est invisible en pratique, peu importe la qualite du code serveur. Les tests unitaires du selecteur serveur ne remplacent pas cette verification: ils prouvent que la logique est correcte SI on l appelle, pas qu elle est appelee.
